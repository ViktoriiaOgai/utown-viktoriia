...
export default function EstablishmentCategoriesPage() {
    ...
    api
      ...
      .then((res) => {
        ...
        setAllItems(
          list.filter(
            (item) =>
              !item.establishmentId || String(item.establishmentId) === String(establishmentId)
          )
        );
      })
      .catch(() => {
  ...
  const handlePriorityChange = (categoryId: number, value: string) =>
    setAllItems((prev) =>
      prev.map((item) =>
        item.id === categoryId ? { ...item, priority: Number(value || 0) } : item
      )
    );
  ...
  const handleEdit = async (item: CategoryRow) => {
    ...
    try {
      ...
      setAllItems((prev) =>
        prev.map((category) =>
          category.id === item.id ? { ...category, name: nextName.trim() } : category
        )
      );
    } catch (err) {
      console.error(err);
    ...
    try {
      await Promise.all(
        selectedIds.map((categoryId) =>
          api.delete(`/admin/categories/${categoryId}`).catch(() => null)
        )
      );
        ...
        <div className="categories__inner">
          <div className="categories__header">
            <div className="categories__header-left">
              ...
              <button
                type="button"
                className={`categories__add-btn${hoverAddPos ? " categories__add-btn--active" : ""}`}
                onClick={() => {
                  if (!establishmentId) return;
                  navigate(`/admin/establishments/${establishmentId}/positions/add`);
                }}
              ...
              <button
                type="button"
                className={`categories__add-btn${hoverAddCat ? " categories__add-btn--active" : ""}`}
                onClick={() => {
                  if (!establishmentId) return;
                  navigate(`/admin/establishments/${establishmentId}/positions/categories/add`);
                }}
          ...
          <div className="categories__toolbar">
            <div className="categories__breadcrumb">
              <span className="categories__breadcrumb-link" onClick={() => navigate("/admin/home")}>
                Home
              </span>
              <span className="categories__breadcrumb-sep">/</span>
              <span
                className="categories__breadcrumb-link"
            ...
            <div className="categories__actions">
              <select className="categories__select" defaultValue="Filter">
              ...
              <button type="button" className="categories__apply-btn" onClick={handleApply}>
                Apply
              </button>
            </div>
          </div>
          ...
          <div className="categories__table">
            ...
            {pageItems.map((item) => (
              <div key={item.id} className="categories__table-row">
                ...
                <div>
                  <input
                    className="categories__priority-input"
                    value={String(item.priority)}
                    onChange={(e) => handlePriorityChange(item.id, e.target.value)}
                    onBlur={() => handlePrioritySave(item)}
                  />
                </div>
                <div
                  className={`categories__edit-btn${savingId === item.id ? " categories__edit-btn--saving" : ""}`}
                  onClick={() => {
          ...
          <div className="categories__pagination">
            <div className="categories__pagination-inner">
              <button
                ...
                className={`categories__page-btn${prevDisabled ? " categories__page-btn--disabled" : ""}`}
                onClick={() => goTo(page - 1)}
              >
                Prev
              </button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  type="button"
                  onClick={() => goTo(p)}
                  className={`categories__page-btn${p === page ? " categories__page-btn--active" : ""}`}
                >
                  {p}
                </button>
              ))}
              <button
                ...
                className={`categories__page-btn${nextDisabled ? " categories__page-btn--disabled" : ""}`}
                onClick={() => goTo(page + 1)}
              >
                Next
              </button>
          ...
          <div className="categories__footer">
            Establishment id: {String(establishmentId || "")}
          </div>
        </div>
      </div>
    </MainLayout>
