        ...
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          <div style={{ marginTop: 18, display: 'flex', justifyContent: 'center' }}>
            <div style={{ width: 540, border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff', padding: 22 }}>
              ...
              <div style={{ marginBottom: 14 }}><div style={labelStyle}>Position Name</div><input placeholder="Name" style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} /></div>
              <div style={{ marginBottom: 14 }}><div style={labelStyle}>Description</div><textarea placeholder="Beef, zucchini, celery, cheese, pepper, cheese edges, etc." value={description} onChange={(e) => setDescription(e.target.value)} style={{ width: '100%', minHeight: 92, borderRadius: 4, border: '1px solid #cbd5e1', padding: '10px 12px', fontSize: 13, outline: 'none', resize: 'none', background: '#fff' }} /></div>
              <div style={{ marginBottom: 14 }}><div style={labelStyle}>Price</div><input value={price} onChange={(e) => setPrice(e.target.value)} style={inputStyle} /></div>
          ...
          <div style={{ marginTop: 26, display: 'flex', justifyContent: 'center', gap: 14 }}>
            ...
            <button type="button" onClick={handleSave} disabled={isSaving} style={{ height: 36, minWidth: 160, borderRadius: 4, border: 'none', background: '#111', color: '#fff', fontWeight: 700, cursor: isSaving ? 'not-allowed' : 'pointer', opacity: isSaving ? 0.7 : 1 }}>{isSaving ? 'Saving...' : 'Save'}</button>
          </div>
        </div>
      </div>
    </MainLayout>
